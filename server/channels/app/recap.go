...
func (a *App) ProcessRecapChannel(rctx request.CTX, recapID, channelID, userID, agentID string) (*model.RecapChannelResult, *model.AppError) {
	...
	skip, reason, appErr := a.runGuardedRecapWillBeProcessed(rctx, channel)
	if appErr != nil {
		return nil, appErr
	}
	if skip {
		// Skip this channel: report success with no output, do not write a recap row. The worker
		// treats Success=false as failure (logs warning); Success=true with MessageCount=0 means
		// "handled, nothing to do." We're debug logging the rejection reason in case the operator
		// is confused.
		rctx.Logger().Debug("Recap channel skipped by plugin", mlog.String("recap_id", recapID), mlog.String("channel_id", channelID), mlog.String("user_id", userID), mlog.String("reason", reason))
		result.Success = true
		return result, nil
	}
