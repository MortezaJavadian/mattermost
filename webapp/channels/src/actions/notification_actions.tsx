 ...
 */
export function isDesktopSoundEnabled(
    channelMember: ChannelMembership | undefined,
    user: UserProfile | undefined,
) {
    const soundInChannelMemberNotifyProps =
        channelMember?.notify_props?.desktop_sound;
    ...
    if (soundInChannelMemberNotifyProps === DesktopSound.DEFAULT) {
        return soundInUserNotifyProps
            ? soundInUserNotifyProps === "true"
            : true;
    }
 ...
 */
export function getDesktopNotificationSound(
    channelMember: ChannelMembership | undefined,
    user: UserProfile | undefined,
) {
    const notificationSoundInChannelMember =
        channelMember?.notify_props?.desktop_notification_sound;
    const notificationSoundInUser =
        user?.notify_props?.desktop_notification_sound;

    if (
        notificationSoundInChannelMember &&
        notificationSoundInChannelMember !== DesktopNotificationSounds.DEFAULT
    ) {
        return notificationSoundInChannelMember;
    }

    if (
        notificationSoundInUser &&
        notificationSoundInUser !== DesktopNotificationSounds.DEFAULT
    ) {
...
export function sendDesktopNotification(
...
): ActionFuncAsync<NotificationResult> {
    return async (dispatch, getState) => {
        ...
        const channel =
            makeGetChannel()(state, post.channel_id) ||
            ({
                id: post.channel_id,
        ...
        const url = isCrtReply
            ? getPermalinkURL(updatedState, teamId, post.id)
            : getChannelURL(updatedState, channel, teamId);

        // Allow plugins to change the notification, or re-enable a notification
        const args: NotificationHooksArgs = {
            title,
            body,
            silent: !desktopSoundEnabled,
            soundName,
            url,
            notify: true,
        };
        ...
        if (hookResult.error) {
            ...
            return {
                data: {
                    status: "error",
                    reason: "desktop_notification_hook",
                    data: String(hookResult.error),
                },
            };
        }
        ...
        const result = dispatch(
            notifyMe(
                argsAfterHooks.title,
                argsAfterHooks.body,
                channel.id,
                teamId,
                argsAfterHooks.silent,
                argsAfterHooks.soundName,
                argsAfterHooks.url,
            ),
        );
...
const getNotificationTitle = (
    channel: Pick<Channel, "type" | "display_name">,
    msgProps: NewPostMessageProps,
    isCrtReply: boolean,
) => {
    let title = Utils.localizeMessage({
        id: "channel_loader.title",
...
const getNotificationBody = (
    ...
    const attachments = isMessageAttachmentArray(post.props?.attachments)
    ...
    if (strippedMarkdownNotifyText.length === 0) {
        if (msgProps.image) {
            ...
            body += Utils.localizeMessage({
                id: "channel_loader.postedImage",
                defaultMessage: " posted an image",
            });
        } else {
            body += Utils.localizeMessage({
                id: "channel_loader.posted",
                defaultMessage: " posted a message",
            });
        }
    } else {
...
function shouldSkipNotification(
    ...
    if (state.views.browser.focused) {
        ...
        if (
            !isCrtReply &&
            !focusedPopout.threadId &&
            focusedPopout.channelId === channelId
        ) {
            return {
                status: "not_sent",
                reason: "channel_is_open",
...
export function notifyMe(
    ...
    return async (dispatch) => {
        ...
        if (isDesktopApp()) {
            const result = await DesktopApp.dispatchNotification(
                title,
                body,
                channelId,
                teamId,
